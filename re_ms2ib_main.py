...
from re_ms2ib_model import ReMS2IB
...
def train(args, model, tokenizer, processor, device, n_gpu, results={}):
    ...
    for epoch_num in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(train_iter):
            ...
            input_ids, input_mask, valid_ids, segment_ids, label_ids, e1_mask, e2_mask, dep_type_matrix,char_sequence = batch

            loss = model(input_ids, segment_ids, input_mask, label_ids, e1_mask=e1_mask, e2_mask=e2_mask,
                         valid_ids=valid_ids, dep_adj_matrix=dep_type_matrix, dep_type_matrix=dep_type_matrix,char_sequence=char_sequence)
            if n_gpu > 1:
        ...
        if args.local_rank == -1 or torch.distributed.get_rank() == 0 or args.world_size <= 1:
            ...
            output_dir = os.path.join(args.output_dir, "best-model")
...
def evaluate(args, model, tokenizer, processor, device, mode="test", output_dir='./'):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            logits = model(input_ids, segment_ids, input_mask, e1_mask=e1_mask, e2_mask=e2_mask,
                           ...
                           valid_ids=valid_ids,char_sequence=char_sequence)
    ...
    if args.task_name == 'semeval':
        ...
        with open(f"semeval.result-beta.txt-{args.tag}",'a') as fp:
            fp.write(str(result))
            fp.write('\n')
    else:
        result = {
...
def get_args():
    ...
    parser.add_argument('--beta1', type=float, default=0.0001)
...
def train_func(args):
    ...
    processor.prepare_char_dict(args.data_dir)
    chars = processor.char_dict.keys()
    chars_num = len(chars)
    ...
    config = BertConfig.from_json_file(os.path.join(args.model_path, "bert_config.json"))
    ...
    config.__dict__["IB"] = args.IB
    config.__dict__["beta1"] = args.beta1
    config.__dict__["beta2"] = args.beta2
    config.__dict__["char_num"] = chars_num
    config.__dict__["max_word_length"] = processor.max_word_length
    # model = ReAgcn(config)
    model = ReMS2IB.from_pretrained(args.model_path, config=config)
