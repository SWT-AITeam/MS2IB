...
class REDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        char_sequence = torch.tensor(self.data[index]["char_sequence"], dtype=torch.long)
        ...
        return input_ids,input_mask,valid_ids,segment_ids,label_ids,e1_mask_ids,e2_mask_ids, dep_type_matrix,char_sequence
...
class RE_Processor():
    ...
    def prepare_labels_dict(self, data_dir):
        ...
        self.labels_dict = labels_dict
    def prepare_char_dict(self, data_dir):
        keys_frequency_dict = defaultdict(int)
        for flag in ["train", "test", "dev"]:
            datafile = os.path.join(data_dir, '{}.txt'.format(flag))
            if os.path.exists(datafile) is False:
            ...
            for data in all_data:
                for word in data['words']:
                    ...
                    for char in word:
                        keys_frequency_dict[char] += 1
        keys_dict = {"[UNK]": 0, "[PAD]": 1}

        for key, freq in sorted(keys_frequency_dict.items(), key=lambda x: x[1], reverse=True):
            keys_dict[key] = len(keys_dict)
        self.char_dict = keys_dict
    def read_features(self, data_dir, flag):
        ...
        for text_data,dep_info in zip(all_text_data, all_dep_info):
            ...
            char_sequence = []
            for word in dep_instance_parser.words:
                for char in word:
                    char_sequence.append(self.char_dict.get(char,0))
        
                if len(word)<self.max_word_length:
                    for i in range(self.max_word_length-len(word)):
                        char_sequence.append(self.char_dict.get("[PAD]"))
    ...
    def convert_examples_to_features(self, examples, tokenizer, max_seq_length):
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            char_sequence+=[1]*((max_seq_length+1)*self.max_word_length-len(example['char_sequence']))
